...
public class DamageLabel extends LabelEntity {
    public DamageLabel(String text, double x, double y, double width, double height) {
        super(text, x, y, width, height);
        this.addComponent(new TimeComponent(500L));
        this.getComponent(TimeComponent.class).setLastOccurrence();

        Platform.runLater(() -> {
            this.getComponent(LabelComponent.class).getUIElement().setStyle("-fx-font-weight: bold; -fx-font-size: 36px; -fx-text-fill: red;");
        });
