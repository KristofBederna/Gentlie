...
public class MainScene extends GameScene {
    ...
    private void declareEntities() {
        LabelEntity label = new LabelEntity("GENTLIE", DisplayConfig.resolution.first() / 2 - 70, DisplayConfig.resolution.second() / 2 - 250, 200, 0);

        ButtonEntity start = new ButtonEntity("Start Game", DisplayConfig.resolution.first() / 2 - 100, DisplayConfig.resolution.second() / 2 - 150, 200, 80, () -> SystemHub.getInstance().getSystem(SceneManagementSystem.class).requestSceneChange(new SaveCreatorScene(new BorderPane(), DisplayConfig.resolution.first(), DisplayConfig.resolution.second())));
        ButtonEntity continueGame = new ButtonEntity("Continue Game", DisplayConfig.resolution.first() / 2 - 100, DisplayConfig.resolution.second() / 2 - 50, 200, 80, () -> SystemHub.getInstance().getSystem(SceneManagementSystem.class).requestSceneChange(new LoadSelectorScene(new BorderPane(), DisplayConfig.resolution.first(), DisplayConfig.resolution.second())));
        ButtonEntity settings = new ButtonEntity("Settings", DisplayConfig.resolution.first() / 2 - 100, DisplayConfig.resolution.second() / 2 + 50, 200, 80, () -> SystemHub.getInstance().getSystem(SceneManagementSystem.class).requestSceneChange(new SettingsScene(new BorderPane(), DisplayConfig.resolution.first(), DisplayConfig.resolution.second())));
        ButtonEntity exit = new ButtonEntity("Exit", DisplayConfig.resolution.first() / 2 - 100, DisplayConfig.resolution.second() / 2 + 150, 200, 80, () -> System.exit(0));
