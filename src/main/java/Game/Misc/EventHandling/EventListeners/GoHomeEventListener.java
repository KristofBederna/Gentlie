...
public class GoHomeEventListener implements EventListener<GoHomeEvent> {
    ...
    public void onEvent(GoHomeEvent event) {
        ...
        if (KeyboardInputHandler.getInstance().isKeyPressed(KeyCode.E))
            SystemHub.getInstance().getSystem(SceneManagementSystem.class).requestSceneChange(new HomeIslandScene(new BorderPane(), 1920, 1080, new Point(14* Config.tileSize-Config.tileSize*0.25-1, 2*Config.tileSize+Config.tileSize*0.25-1)));
    }
